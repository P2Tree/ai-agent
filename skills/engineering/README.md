...
编码行为准则，减少 LLM 写代码时的常见错误。四条原则：编码前先想清楚（别想当然）、简洁优先（别过度设计）、精准修改（别碰无关代码）、目标驱动（验证了再说做完）。

**什么时候用：** 写代码、审查代码、重构代码时。

**触发方式：** 作为行为准则始终生效，编写、审查、重构代码时自动应用——不是按需触发的技能，而是内置的编码纪律。

**参考：** andrej-karpathy-skills:karpathy-guidelines
